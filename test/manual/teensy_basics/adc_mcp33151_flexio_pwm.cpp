...
// based on https://forum.pjrc.com/threads/66201-Teensy-4-1-How-to-start-using-FlexIO
...
void setup() {
  ...
  flexio->port().TIMCTL[_pwm_timer_idx] =
      FLEXIO_TIMCTL_PINSEL(PIN_FLEX_CNVST) | FLEXIO_TIMCTL_TIMOD(2) | FLEXIO_TIMCTL_PINCFG(3);
  ...
  flexio->port().TIMCTL[_clk_timer_idx] = FLEXIO_TIMCTL_TRGSEL(2 * PIN_FLEX_CNVST) | FLEXIO_TIMCTL_TRGSRC |
                                          FLEXIO_TIMCTL_TRGPOL | FLEXIO_TIMCTL_PINCFG(3) |
                                          FLEXIO_TIMCTL_PINSEL(PIN_FLEX_CLK) | FLEXIO_TIMCTL_TIMOD(1);
  // Configure timer (https://www.pjrc.com/teensy/IMXRT1060RM_rev3.pdf p.3002)
  flexio->port().TIMCFG[_clk_timer_idx] =
      FLEXIO_TIMCFG_TIMOUT(1) | FLEXIO_TIMCFG_TIMDIS(2) | FLEXIO_TIMCFG_TIMENA(6);
  ...
  flexio->port().SHIFTCTL[_miso_shifter_idx] = FLEXIO_SHIFTCTL_TIMSEL(_clk_timer_idx) |
                                               FLEXIO_SHIFTCTL_TIMPOL | FLEXIO_SHIFTCTL_PINSEL(PIN_FLEX_MISO) |
  ...
  for (decltype(_miso_shifter_chain_count) chain_idx = 1; chain_idx < _miso_shifter_chain_count + 1;
       chain_idx++) {
  ...
  // TODO: Make a simpler test example with just a timer and a shifter shifting zeros or something and
  // triggering DMA
  ...
  */
  ...
  dmaChannel.TCD->CSR &=
      ~(DMA_TCD_CSR_DREQ); // do not disable the channel after it completes - so it just keeps going
  dmaChannel.TCD->CSR |=
