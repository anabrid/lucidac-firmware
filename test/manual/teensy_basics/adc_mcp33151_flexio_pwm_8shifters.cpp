...
void setup() {
  ...
  for (auto PIN_MISO : PINS_MISO) {
    ...
    flexio->port().SHIFTCTL[shifter_idx] = FLEXIO_SHIFTCTL_TIMSEL(_clk_timer_idx) | FLEXIO_SHIFTCTL_TIMPOL |
                                           FLEXIO_SHIFTCTL_PINSEL(pin_miso_flex_idx) | FLEXIO_SHIFTCTL_SMOD(1);
  ...
  dma.TCD->DOFF = 4;
  dma.TCD->ATTR_DST = B00101010; // [5bit MOD, 00101=5lower bits of address may change, 32bytes = 4*2*32bit
                                 // buffer][3bit SIZE, 010=32bit]
