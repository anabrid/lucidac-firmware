...
void test_coeff() {
  auto *identityblock = (MIntBlock *)(cluster.m1block);
  ...
  TEST_ASSERT(
      cluster.ublock->connect(identityblock->M1_OUTPUT(0), UBlock::OUTPUT_IDX_RANGE_TO_ADC()[adc_channel]));

  // Route the result to an ACL output
  TEST_ASSERT(cluster.ublock->connect(identityblock->M1_OUTPUT(0), UBlock::IDX_RANGE_TO_ACL_OUT(5)));

  // Route identity block
  TEST_ASSERT(cluster.route(blocks::UBlock::ALT_SIGNAL_REF_HALF_INPUT, coeff, 0, identityblock->M1_INPUT(0)));

  cluster.write_to_hardware();
  delayMicroseconds(100);

  for (float c = -1; c <= 1; c += 0.001) {
    TEST_ASSERT(cluster.cblock->set_factor(coeff, c));
  ...
  sprintf(buffer, "Coeff %d | ACD %d | Mean Diff = %f | Max Diff  = %f", coeff, adc_channel,
          sum_diff / num_diff, max_diff);
  TEST_MESSAGE(buffer);
...
void setup() {
  ...
  RUN_TEST(test_init);
  char buffer[128];
  sprintf(buffer, "Sweep through [-1, 1] for each Coefficient and check if actual value is within %f",
          tolerance);
  TEST_MESSAGE(buffer);
