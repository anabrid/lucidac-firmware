...
void test_coeff() {
  ...
  for(uint8_t coeff=0; coeff<32; coeff++) {
    ...
    char buffer[128];
    sprintf(buffer, "Coeff%d ADC%d", coeff, adc_channel);
    TEST_MESSAGE(buffer);
    ...
    luci.ublock->connect(identityblock->M1_OUTPUT(0), UBlock::OUTPUT_IDX_RANGE_TO_ADC()[adc_channel]);

    if (ANABRID_TESTS_USE_ACL_OUT) {
      // Route the result to an ACL output
      luci.ublock->connect(identityblock->M1_OUTPUT(0), UBlock::IDX_RANGE_TO_ACL_OUT(5));
    }

    // Route identity block
    luci.route(blocks::UBlock::ALT_SIGNAL_REF_HALF_INPUT, coeff, 0, identityblock->M1_INPUT(0));
    
    luci.write_to_hardware();
    delayMicroseconds(100);

    for (float c = -1; c <= 1; c += 0.001) {
      luci.cblock->set_factor(coeff,c);
      luci.cblock->write_to_hardware();
      delayMicroseconds(10);

      float adc_value = daq_.sample()[adc_channel];
      TEST_ASSERT_FLOAT_WITHIN(ANABRID_TESTS_TARGET_PRECISION, c, -adc_value);

      sprintf(buffer, "C = %f, ACD = %f", c, -adc_value);
      TEST_MESSAGE(buffer);
