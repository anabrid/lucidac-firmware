...
namespace utils {
  ...
  public:
    ...
    virtual size_t write(uint8_t b) override {
      ...
      bool success = true;
      for(auto& target : targets) if(target) success &= target->write(b) == 1;
    ...
    size_t write(const uint8_t *buffer, size_t size) override {
      ...
      size_t i = 0;
      bool success = true;
      for(auto& target : targets) if(target) success &= qindesign::network::util::writeFully(*target, buffer, size) == size;
