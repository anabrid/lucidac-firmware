...
void convertFromJson(JsonVariantConst ipjson, IPAddress &ip) { ip.fromString(ipjson.as<const char *>()); }

void convertToJson(const IPAddress &ip, JsonVariant ipjson) { ipjson.set(toString(ip)); }

void utils::convertToJson(const MacAddress &mac, JsonVariant macjson) { macjson.set(toString(mac)); }
