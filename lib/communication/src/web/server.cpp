...
FLASHMEM void serve_static(const web::StaticFile &file, awot::Response &res) {
  ...
  for (uint32_t written = 0; written != file.size;) {
...
FLASHMEM void websocket_upgrade(awot::Request &req, awot::Response &res) {
  ...
  if (!req.get("Connection"))
    ERR("Missing 'Connection' header");
  if (!contains(req.get("Connection"), "Upgrade"))
    ERR("Connection header contains not 'Upgrade'");
  if (!req.get("Upgrade"))
    ERR("Missing 'Upgrade' header");
  if (!equals(req.get("Upgrade"), "websocket"))
    ERR("Upgrade header is not 'websocket'");
  if (!req.get("Sec-WebSocket-Version"))
    ERR("Missing Sec-Websocket-Version");
  if (!equals(req.get("Sec-WebSocket-Version"), "13"))
    ERR("Only Sec-Websocket-Version=13 supported");
...
FLASHMEM void onWebsocketMessageCallback(websockets::WebsocketsClient &wsclient, websockets::WebsocketsMessage msg) {
  ...
  if (!msg.isText()) {
    LOG_ALWAYS("webSockets: Ignoring non-text message");
    return;
  }

  std::string envelope_out_str;
  msg::JsonLinesProtocol::get().process_string_input(msg.data(), envelope_out_str,
                                                     wsclient.client()->context->user_context);
  ...
  for (auto client = clients.begin(); client != clients.end(); client++) {
    ...
    if (client->socket.connected()) {
      if (client->ws.available()) {
        ...
        LOG5("Web client ", client_idx, ", timed out after ", net::StartupConfig::get().connection_timeout_ms,
             " ms of idling");
