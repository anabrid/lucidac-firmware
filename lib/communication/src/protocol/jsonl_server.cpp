...
void msg::JsonlServer::begin() {
    server.begin(net::StartupConfig::get().jsonl_port);
}
  ...
  for(auto client = clients.begin(); client != clients.end(); client++) {
    ...
    if(client->socket) {
      if(client->socket.available() > 0) {
        ...
        LOG5("Client ", client_idx, ", timed out after ", net::StartupConfig::get().connection_timeout_ms, " ms of idling");
        client->socket.stop();
      }
    } else {
