...
int net::auth::Gatekeeper::login(JsonObjectConst msg_in, JsonObject &msg_out, net::auth::AuthentificationContext& user_context) {
...
int net::auth::Gatekeeper::lock_acquire(JsonObjectConst msg_in, JsonObject &msg_out, AuthentificationContext &user_context) {
...
int net::auth::Gatekeeper::lock_release(JsonObjectConst msg_in, JsonObject &msg_out, AuthentificationContext &user_context) {
  if(!enable_users || user_context.user() == lock.holder || user_context.can_do(SecurityLevel::RequiresAdmin)) {
    lock.force_unlock();
    return 0;
  }
}
